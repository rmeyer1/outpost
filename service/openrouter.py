...
Responsibilities:
  - read the OpenRouter key from its secure location ON THIS HOST ONLY
    (service/openrouter.py runs on the Outpost host, never inside a container).
...
The key is NEVER logged, printed, or copied off this host. It is registered
...
def read_key(spend_cfg: dict) -> str:
    ...
    path = os.environ.get("CA_OR_KEY_FILE") or spend_cfg.get(
        "openrouter_key_file",
        os.path.expanduser("~/.config/outpost/openrouter.yaml"))
